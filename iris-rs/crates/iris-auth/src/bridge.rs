//! Bridge between iris `AppConfig` and `l42-token-handler`.
//!
...
//! route builder for integration into the iris-web router.
...
/// Convert iris `AppConfig` (loaded from `SECDASH_*` env vars) into the l42
...
/// Auth components ready for integration into the iris-web router.
...
/// Build auth components from iris config.
///
...
mod tests {
    ...
    fn config_conversion_derives_frontend_from_redirect_uri() {
        let app = AppConfig {
            cognito_redirect_uri: "https://iris.lexicone.com/auth/callback".into(),
            ..AppConfig::default()
        };
